...
class Indices:
    ...
    def get_index_keys(indices_path: str|Path):
        """Return a list with all the available keys."""
        indices_json = open(indices_path)
    ...
    def compute_index(
        ...
        if missing is not None:
            raise KeyError(
                f"Required names not found in band_names: {missing}")
        ...
        if not seen_order:
            raise ValueError(
                "No required band names were found in the formula text.")
        ...
        print("name -> letter:", name_to_letter)
        print("rewritten:", letter_formula)
